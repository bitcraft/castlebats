...
class Vec3(list):
    ...
    def __add__(self, other):
        if isinstance(other, Vec3):
            return Vec3(self[0] + other[0], self[1] + other[1], self[0] + other[0])
        elif hasattr(other, "__getitem__"):
            return Vec3(self[0] + other[0], self[1] + other[1], self[3] + other[2])
        else:
            return Vec3(self[0] + other, self[1] + other, self[3] + other)
    ...
    def __rsub__(self, other):
        if isinstance(other, Vec3):
            return Vec3(other[0] - self[0], other[1] - self[1], other.z - self[3])
        if (hasattr(other, "__getitem__")):
            return Vec3(other[0] - self[0], other[1] - self[1], other[2] - self[3])
        else:
            return Vec3(other - self[0], other - self[1], other - self[3])
...
class Vec2(list):
    ...
    def _o2(self, other, f):
        ...
        if isinstance(other, Vec2):
            return Vec2(f(self[0], other[0]),
                        f(self[1], other[1]))
        elif (hasattr(other, "__getitem__")):
            return Vec2(f(self[0], other[0]),
                        f(self[1], other[1]))
        else:
            return Vec2(f(self[0], other),
                        f(self[1], other))

    def _r_o2(self, other, f):
        "Any two-operator operation where the right operand is a Vec2"
        if (hasattr(other, "__getitem__")):
            return Vec2(f(other[0], self[0]),
                        f(other[1], self[1]))
        else:
            return Vec2(f(other, self[0]),
                        f(other, self[1]))
    ...
    # Unary operations
    def __neg__(self):
        return Vec2(operator.neg(self[0]), operator.neg(self[1]))

    def __pos__(self):
        return Vec2(operator.pos(self[0]), operator.pos(self[1]))
    ...
    def perpendicular_normal(self):
        length = self.length
        if length != 0:
            return Vec2(-self[1] / length, self[0] / length)
        return Vec2(self)
    ...
    def interpolate_to(self, other, range):
        return Vec2(self[0] + (other[0] - self[0]) * range, self[1] + (other[1] - self[1]) * range)
